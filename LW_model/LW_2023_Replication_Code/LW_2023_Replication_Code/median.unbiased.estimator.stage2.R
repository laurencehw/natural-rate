...
#------------------------------------------------------------------------------#
median.unbiased.estimator.stage2 <- function(y, x, kappa.vec) {
    t.end <- dim(x)[1]
    stat <- rep(0, t.end-2*4+1)
    w <- diag(1/(kappa.vec^2))
    for (i in 4:(t.end-4)) {
        xr <- cbind(x, c(rep(0, i), rep(1, t.end-i)))
        xi <-  solve(t(xr)%*%w%*%xr)
        b <- solve(t(xr)%*%w%*%xr,t(xr)%*%w%*%y)
        s3 <- sum(w%*%(y-xr%*%b)^2)/(sum(w)-(dim(xr)[2]))
