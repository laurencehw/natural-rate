...
kalman.standard.errors <- function(t.end, states, theta, y.data, x.data, stage,
  ...
  # Sets date for inclusion of COVID indicator in LW model. If 2020:Q1 or later, compute SE for phi; otherwise, will omit
  ...
  while (good.draws < niter) {
    ...
    if ( (theta.i[param.num["a_3"]] <= a.r.constraint) &
         (theta.i[param.num["b_3"]] >= b.y.constraint) &
         (theta.i[param.num["a_1"]] + theta.i[param.num["a_2"]] < 1) ){
      ...
      if (theta.i[param.num["b_3"]] < b.y.constraint) {
        excluded.draw.counter.b.y <- excluded.draw.counter.b.y + 1
      }
      if ((theta.i[param.num["a_1"]] + theta.i[param.num["a_2"]]) >= 1) {
